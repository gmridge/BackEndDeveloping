...
router.get('/filter', (req, res) => {
    ...
    for (let key in req.query) {
        matchingFriends = matchingFriends.filter(friend => friend[key] == req.query[key]);
    }
...
router.get('/letter', (req, res) => {
    ...
    if (filterLetter) {
        let matchingFriends = myFriends.filter(friend => friend.letter == filterLetter);
        if (matchingFriends.length > 0) {
...
router.get('/:id', (req, res) => {
    ...
    friends ? res.status(200).json({result: friends})
    : res.status(404).json({
        result:
